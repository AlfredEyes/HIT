...
class Hiera(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        print("\nHiera Forward Pass Details:")
        print(f"Input shape: {x.shape}")
        
        # Patch embedding
        x = self.patch_embed(x, mask=mask.view(x.shape[0], 1, *self.mask_spatial_shape) if mask is not None else None)
...
@pretrained_model({
    "mae_k400_ft_k400": "https://dl.fbaipublicfiles.com/hiera/hiera_tiny_16x224.pth",
    "mae_k400": "https://dl.fbaipublicfiles.com/hiera/mae_hiera_tiny_16x224.pth",
}, default="mae_k400_ft_k400")
def hiera_tiny_16x224(num_classes: int = 2, **kwdargs):
    default_args = {
        "num_classes": 2,
        "input_size": (16, 224, 224),  # T x H x W
        "embed_dim": 96,
        "num_heads": 1,
        "stages": (1, 2, 7, 2),  #? Configuration tiny
        "q_stride": (1, 2, 2),
    ...
    default_args.update(kwdargs)
    return Hiera(**default_args)


@pretrained_model({
    "mae_k400_ft_k400": "https://dl.fbaipublicfiles.com/hiera/hiera_small_16x224.pth",
    "mae_k400": "https://dl.fbaipublicfiles.com/hiera/mae_hiera_small_16x224.pth",
}, default="mae_k400_ft_k400")
def hiera_small_16x224(num_classes: int = 2, **kwdargs):
    default_args = {
        "num_classes": 2,
        "input_size": (16, 224, 224),  # T x H x W
        "embed_dim": 96,
        "num_heads": 1,
        "stages": (1, 2, 11, 2),  #? Configuration small
        "q_stride": (1, 2, 2),
        "mask_unit_size": (1, 8, 8),
        "patch_kernel": (3, 7, 7),
        "patch_stride": (2, 4, 4),
...
def hiera_base_16x224(num_classes: int = 2, **kwdargs):
    default_args = {
        "num_classes": 2,  # K400 has 400 classes
        "input_size": (16, 224, 224), # T x H x W
        "q_stride": (1, 2, 2), # q_stride for each stage
        "mask_unit_size": (1, 8, 8),
        "patch_kernel": (3, 7, 7),
        "patch_stride": (2, 4, 4),
        "patch_padding": (1, 3, 3),
        "sep_pos_embed": True, # separate spatial and temporal positional embeddings
    }
    # Only override defaults if explicitly provided in kwdargs
    default_args.update(kwdargs)
    return Hiera(**default_args)
